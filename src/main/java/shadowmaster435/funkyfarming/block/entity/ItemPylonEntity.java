...
public class ItemPylonEntity extends BlockEntity {
    ...
    public ItemPylonEntity(BlockPos pos, BlockState state) {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, ItemPylonEntity be) {
        if (be.loaddelay < 10) {
            ++be.loaddelay;
        } else {
            for (int i = 0; i < be.xlist.size(); ++i) {
                BlockPos bpos = new BlockPos(be.xlist.get(i), be.ylist.get(i), be.zlist.get(i));
                if (world.getBlockState(bpos).getBlock() != FFBlocks.ITEM_PYLON) {
